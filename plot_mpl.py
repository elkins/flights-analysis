...
def plot_map(
    ...
    # Define CSV columns
    csv_cols = data_config.get(
        "columns", ["dep_lat", "dep_lon", "arr_lat", "arr_lon", "nb_flights", "CO2"]
    )
    ...
    norm = PowerNorm(gamma=gamma, vmin=routes["nb_flights"].min(), vmax=routes["nb_flights"].max())
...
def main() -> None:
    ...
    parser.add_argument("-i", "--input", type=Path, help="Input CSV file (default: data.csv)")
    parser.add_argument(
        "-o", "--output", type=Path, help="Output PNG file (default: flights_map_mpl.png)"
    )
    parser.add_argument(
        "-c", "--config", type=Path, help="Configuration YAML file (default: config.yaml)"
    )
    parser.add_argument(
        "--color-mode", choices=["screen", "print"], help="Color scheme: screen or print"
    )
    parser.add_argument(
        "--absolute", action="store_true", help="Use absolute color scaling based on flight counts"
    )
    ...
    # Generate the map
    plot_map(input_file, output_file, config, color_mode=args.color_mode, absolute=args.absolute)
