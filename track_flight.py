...
class FlightTracker:
    ...
    def __init__(self, api_key: str | None = None):
        ...
        self.session.headers.update({"User-Agent": "FlightTracker/1.0"})
    ...
    def track_continuous(
        self, callsign: str, interval_seconds: int = 30, max_updates: int | None = None
    ) -> list[FlightPosition]:
        """
    ...
    def plot_flight_path(self, output_path: Path | None = None) -> None:
        ...
        positions_with_coords = [
            p for p in self.flight_history if p.lat is not None and p.lon is not None
        ]
        ...
        with open(temp_csv, "w", newline="") as f:
            ...
            for i in range(len(positions_with_coords) - 1):
                ...
                writer.writerow(
                    [f"{p1.lat:.5f}", f"{p1.lon:.5f}", f"{p2.lat:.5f}", f"{p2.lon:.5f}", 1, 50.0]
                )
...
Examples:
  ...
  python track_flight.py UA262 --follow --updates 20 --plot
    ...
    parser.add_argument("callsign", help="Flight callsign (e.g., UA262, UAL262)")

    parser.add_argument("-o", "--output", type=Path, help="Output CSV file for flight track")

    parser.add_argument("--follow", action="store_true", help="Continuously track the flight")

    parser.add_argument(
        "--interval", type=int, default=30, help="Update interval in seconds (default: 30)"
    )

    parser.add_argument(
        "--updates", type=int, help="Maximum number of updates (default: unlimited)"
    )

    parser.add_argument("--plot", action="store_true", help="Plot the flight path")

    parser.add_argument("--api-key", help="ADS-B Exchange RapidAPI key (optional)")
    ...
    if args.follow:
        ...
        tracker.track_continuous(
            args.callsign, interval_seconds=args.interval, max_updates=args.updates
        )
    else:
        # Single lookup
