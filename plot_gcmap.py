...
def generate_map(input_file: Path, output_file: Path, config: dict) -> None:
    ...
    gradient_config = gcmap_config.get(
        "gradient",
        [
    ...
    gradient_tuples = tuple((g["position"], *g["color"]) for g in gradient_config)
...
def main() -> None:
    ...
    parser.add_argument(
        "-o", "--output", type=Path, help="Output PNG file (default: flights_map_gcmap.png)"
    )
    parser.add_argument(
        "-c", "--config", type=Path, help="Configuration YAML file (default: config.yaml)"
    )
    parser.add_argument("--width", type=int, help="Map width in pixels")
    parser.add_argument("--height", type=int, help="Map height in pixels")
